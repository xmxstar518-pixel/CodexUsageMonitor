...
- 支持简体中文和英文。
- 首次启动自动检测 macOS 首选语言，手动切换后会记住选择。
- 浮窗支持边缘吸附、鼠标缩放和内容自适应。
- 底部提供 20%–100% 无极透明度滑块，轨道以透明到实色渐变展示。
- 固定与非固定状态使用不同颜色和图标，并真实切换窗口置顶层级。
- 语言切换位于紧凑标题栏，菜单栏弹层底部按钮保持完整可见。
- 支持退出、应用内卸载和独立卸载脚本。
- 不读取或保存 ChatGPT Token，复用本机 Codex 登录状态。
- 兼容空限额、已达到上限和工作区 Credits，不猜测不同计划的额度数值。
- 进度条根据剩余用量动态渐变：充足时为薄荷绿/绿色，中低时转为黄色/橙色，接近耗尽时变为红色。
...
首次启动时，应用读取 macOS 首选语言。中文语言变体显示简体中文，其他语言显示英文。此后可以随时通过地球图标菜单切换语言，应用会记住选择。维护者可通过 `--language=zh-Hans` 或 `--language=en` 测试两套界面。
